...
This document describes how to build the system-dir tarball, needed for the mullet platform to function properly. The archive contains original Android Wear parts (mostly /system/) and recompiled parts for libhybris compatibility (/include/ and /usr/libexec/hal-droid/system/lib/).
...
* M6E69F full system: https://androidfilehost.com/?fid=745425885120748838 or http://www.rootjunkysdl.com/files/Huawei%20Watch/Firmware/mullet-m6e69f-wear-stock.zip
...
```sh
fastboot flash bootloader bootloader-mullet-m6e69f.img
...
cd ../../../lib/hw/
ln -s /usr/libexec/hal-droid/system/lib/hw/hwcomposer.msm8909.so hwcomposer.mullet.so
ln -s /usr/libexec/hal-droid/system/lib/hw/gralloc.msm8909.so gralloc.mullet.so
ln -s /usr/libexec/hal-droid/system/lib/hw/memtrack.msm8909.so memtrack.mullet.so
cd ../../../
```
...
```sh
android-mullet/libhybris/utils/extract-headers.sh -v 7.1.1 -p /usr/include/android android-mullet include
